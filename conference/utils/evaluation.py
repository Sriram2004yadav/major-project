...
def run_quantitative_evaluation(net, cfg, device, run_type: str, enable_mti: bool = False,
    ...
    for step, item in enumerate(dataloader):
        ...
        with torch.no_grad():
            if enable_mti:
                # inference() uses calibrated probs → MTI — no change needed here
                o_seg = net.module.inference(x, edges_mti)
                o_ch  = helpers.get_ch(o_seg, edges_cyclic)
            else:
                # ONLY CHANGE FROM ORIGINAL:
                # Original: logits_ch, logits_seg = net(x, edges_cyclic)
                # Reason:   forward() now returns 3 values; index 0 and 1 are identical
                out       = net(x, edges_cyclic)
                logits_ch  = out[0]
                logits_seg = out[1]
                # out[2] = calibrated_logits (used in inference path, not here)
