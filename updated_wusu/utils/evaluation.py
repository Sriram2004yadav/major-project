...
def run_quantitative_evaluation(net, cfg, device, run_type: str, enable_mti: bool = False,
    ...
    # Use larger batch size for eval since we don't store gradients
    batch_size = cfg.TRAINER.BATCH_SIZE  # Use 8, don't double it!
    dataloader = torch_data.DataLoader(ds, batch_size=batch_size, num_workers=4, shuffle=False, drop_last=False)
    ...
    for step, item in enumerate(dataloader):
        ...
        with torch.no_grad():
            if enable_mti:
                # Old path: Use inference method (which we updated to return refined seg)
                o_seg = net.module.inference(x, edges_mti)
                o_ch = helpers.get_ch(o_seg, edges_for_eval)
            else:
                # --- FIX: UNPACK 3 VALUES ---
                # The model now returns (change, raw_seg, refined_seg)
                logits_ch, logits_seg, refined_seg = net(x, edges_for_eval)
                
                # --- SOTA FIX: Evaluate on the REFINED output ---
                # We use the SOTA output (refined_seg) for metrics, not the raw one!
...
def model_evaluation(net, cfg, device, run_type: str, epoch: float, step: int) -> float:
    ...
    if cfg.DATASET.NAME == 'tscd':
        ...
        # Print results to terminal for immediate feedback
        print(f"--- VALIDATION RESULTS (Epoch {epoch}) ---")
        print(f"F1 Total: {f1:.4f}")
        print(f"F1 Seg:   {f1_seg_cont:.4f}")
        print(f"F1 Change:{f1_ch_cont:.4f}")
