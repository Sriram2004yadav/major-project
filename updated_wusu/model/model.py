...
class ContUrbanCDModel(nn.Module):
    def __init__(self, cfg: CfgNode):
        ...
        # --- SOTA CHANGE: Neural Refiner instead of MTI ---
        # We use hidden_ch=16 for speed and memory efficiency
        self.neural_refiner = modules.NeuralRefiner(in_ch=2, hidden_ch=16) 
    ...
    def forward(self, x: Tensor, edges: Sequence[Tuple[int, int]]) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # Change detection (Raw)
        logits_ch = self.outc_ch(self.decoder_ch(features_ch))
        logits_ch = einops.rearrange(logits_ch, '(b n) c h w -> b n c h w', n=len(edges))

        # --- SOTA CHANGE: Refine outputs End-to-End ---
        # This allows gradients to flow back through the refiner to the decoders
        refined_seg = self.neural_refiner(logits_seg, logits_ch)

        # Return THREE items now: Raw Change, Raw Seg, Refined Seg
        return logits_ch, logits_seg, refined_seg

    def inference(self, x: Tensor, edges: Sequence[Tuple[int, int]]) -> Tensor:
        # During inference, we rely on the refined output
        _, _, refined_seg = self.forward(x, edges)
        return torch.sigmoid(refined_seg).detach()
