...
#
# --- REPLACE the old EvalTestWUSUDataset with this new one ---
#
class EvalTestWUSUDataset(AbstractWUSUDataset):
    """
    Test Evaluation Dataset Class for WUSU.
    This version reads samples_test.json and tiles dynamically.
    """
    def __init__(self, cfg: experiment_manager.CfgNode, site: str = None, tiling: int = None):
        super().__init__(cfg)
        ...
        if len(samples) > 0:
            ...
            first_sample_site = samples[0]['site']
            # Load the first image to check its shape
            temp_img = self.load_gf2_img(first_sample_site, self.dataset, first_sample_idx, self.timestamps[0])
            height, width, _ = temp_img.shape
            print(f"EvalTestWUSUDataset: Detected image size as {height}x{width}. Creating tiles...")
        else:
            height, width = 0, 0
    ...
    def __getitem__(self, index):
        sample = self.samples[index]
        site, index_str, i, j = sample['site'], sample['index'], sample['i'], sample['j']

        # Load 3 images (2015, 2016, 2018)
        images = [self.load_gf2_img(site, self.dataset, index_str, year) for year in self.timestamps]
        images = np.stack(images) # (3, H, W, 4)
        
        # Load 3 segmentation labels
        labels = [self.load_building_label(site, self.dataset, index_str, year) for year in self.timestamps]
        labels = np.stack(labels) # (3, H, W, 1)

        # Crop the full tiles
        images = images[:, i:i + self.crop_size, j:j + self.crop_size]
        labels = labels[:, i:i + self.crop_size, j:j + self.crop_size]

        # Apply transforms (Numpy2Torch)
        images, labels = self.transform((images, labels))

        item = {
            'x': images,
            'y': labels, # WUSU uses 'y' for segmentation
            'index': index_str,
            'i': i,
            'j': j,
        }
